...
class TestResourcesCopy:
    ...
    def test_copy_file_contents_match_bundled(self, tmp_path):
        ...
        static_resources = [
            f for f in ALL_RESOURCE_FILES
            if f not in ("test-writing-policy.md", "specd-orientation.md")
        ]
...
def _bundled_orientation_text():
    """Return the raw text of the bundled specd-orientation.md."""
    resources_dir = importlib.resources.files("specd") / "resources"
    return (resources_dir / "specd-orientation.md").read_text(encoding="utf-8")


def _create_and_read_specd_orientation(cwd):
    """
    Run `specd resources --create` with the given cwd and return the
    content of the created specd-orientation.md.
    """
    target = Path(cwd) / "output"
    target.mkdir(exist_ok=True)
    result = subprocess.run(
    ...
    return (target / "specd-orientation.md").read_text(encoding="utf-8")


class TestSpecdOrientationContent:

    def test_static_content_preserved(self, tmp_path):
        """
        Spec: The created version of specd-orientation.md includes all the static content of the bundled specd-orientation.md that is unaffected by any jinja syntax [resources.md]
        """
        content = _create_and_read_specd_orientation(tmp_path)
        template_text = _bundled_orientation_text()
        paragraphs = _static_paragraphs(template_text)
        assert len(paragraphs) > 0, "Expected at least one static paragraph"
        for paragraph in paragraphs:
            assert paragraph in content, (
    ...
    def test_templates_dir_configured(self, tmp_path):
        """
        Spec: When pyproject.toml provides a `templates` dir, the rendered specd-orientation.md contains: 'Specs are generated from templates in <the configured `templates` dir + one '/'>' [resources.md]
        Spec: When pyproject.toml provides a `templates` dir, the rendered specd-orientation.md contains: 'Agents should leave the files under <the configured `templates` dir + one '/'> alone' [resources.md]
        """
        ...
        content = _create_and_read_specd_orientation(tmp_path)
        assert "Specs are generated from templates in custom_templates/" in content
        assert "Agents should leave the files under custom_templates/ alone" in content

    def test_templates_dir_default(self, tmp_path):
        """
        Spec: When pyproject.toml does not provide a `templates` dir, the rendered specd-orientation.md contains: 'Specs are generated from templates in <the default `templates` dir>' [resources.md]
        Spec: When pyproject.toml does not provide a `templates` dir, the rendered specd-orientation.md contains: 'Agents should leave the files under <the default `templates` dir> alone' [resources.md]
        """
        content = _create_and_read_specd_orientation(tmp_path)
        assert "Specs are generated from templates in templates/" in content
        assert "Agents should leave the files under templates/ alone" in content

    def test_specs_dir_configured(self, tmp_path):
        """
        Spec: When pyproject.toml provides a `specs` dir, the rendered specd-orientation.md contains: 'Canonical specs are under <the configured `specs` dir + one '/'>'. [resources.md]
        Spec: When pyproject.toml provides a `specs` dir, the rendered specd-orientation.md contains: 'They are autogenerated into <the configured `specs` dir + one '/'>' [resources.md]
        Spec: When pyproject.toml provides a `specs` dir, the rendered specd-orientation.md contains: 'Agents only read spec files under <the configured `specs` dir + one '/'>' [resources.md]
        Spec: When pyproject.toml provides a `specs` dir, the rendered specd-orientation.md contains: 'Do not edit spec files under <the configured `specs` dir + one '/'>' [resources.md]
        """
        (tmp_path / "pyproject.toml").write_text(
            "[tool.specd]\nspecs = \"custom_specs\"\n",
            encoding="utf-8",
        ...
        content = _create_and_read_specd_orientation(tmp_path)
        assert "Canonical specs are under custom_specs/" in content
        assert "autogenerated into custom_specs/" in content
        assert "Agents only read spec files under custom_specs/" in content
        assert "Do not edit spec files under custom_specs/" in content

    def test_specs_dir_default(self, tmp_path):
        """
        Spec: When pyproject.toml does not provide a `specs` dir, the rendered specd-orientation.md contains: 'Canonical specs are under <the default `specs` dir>'. [resources.md]
        Spec: When pyproject.toml does not provide a `specs` dir, the rendered specd-orientation.md contains: 'They are autogenerated into <the default `specs` dir>' [resources.md]
        Spec: When pyproject.toml does not provide a `specs` dir, the rendered specd-orientation.md contains: 'Agents only read spec files under <the default `specs` dir>' [resources.md]
        Spec: When pyproject.toml does not provide a `specs` dir, the rendered specd-orientation.md contains: 'Do not edit spec files under <the default `specs` dir>' [resources.md]
        """
        content = _create_and_read_specd_orientation(tmp_path)
        assert "Canonical specs are under specs/" in content
        assert "autogenerated into specs/" in content
        assert "Agents only read spec files under specs/" in content
        assert "Do not edit spec files under specs/" in content

    def test_tests_dir_configured(self, tmp_path):
        """
        Spec: When pyproject.toml provides a `tests` dir, the rendered specd-orientation.md contains: 'Tests that `specd` is aware of are found under <the configured `tests` dir + one '/'>'. [resources.md]
        """
        (tmp_path / "pyproject.toml").write_text(
            "[tool.specd]\ntests = \"custom_tests\"\n",
            encoding="utf-8",
        ...
        content = _create_and_read_specd_orientation(tmp_path)
        assert (
            "Tests that `specd` is aware of are found under custom_tests/"
            in content
        )

    def test_tests_dir_default(self, tmp_path):
        """
        Spec: When pyproject.toml does not provide a `tests` dir, the rendered specd-orientation.md contains: 'Tests that `specd` is aware of are found under <the default `tests` dir>'. [resources.md]
        """
        content = _create_and_read_specd_orientation(tmp_path)
        assert (
    ...
    def test_resources_dir_configured(self, tmp_path):
        """
        Spec: When pyproject.toml provides a `resources` dir, the rendered specd-orientation.md contains: '<the configured `resources` dir + one '/' + spec-writing-policy.md> (how to write spec items)' [resources.md]
        Spec: When pyproject.toml provides a `resources` dir, the rendered specd-orientation.md contains: '<the configured `resources` dir + one '/' + test-writing-policy.md> (reference format and coverage rules)' [resources.md]
        Spec: When pyproject.toml provides a `resources` dir, the rendered specd-orientation.md contains: '<the configured `resources` dir + one '/' + spec-implementation-policy.md> (workflow for implementing specs; unenforced)' [resources.md]
        """
        (tmp_path / "pyproject.toml").write_text(
        ...
        content = _create_and_read_specd_orientation(tmp_path)
        assert (
            "custom_resources/spec-writing-policy.md (how to write spec items)"
            in content
        )
        assert (
            "custom_resources/test-writing-policy.md"
            " (reference format and coverage rules)"
            in content
        )
        assert (
            "custom_resources/spec-implementation-policy.md"
            " (workflow for implementing specs; unenforced)"
            in content
        )

    def test_resources_dir_default(self, tmp_path):
        """
        Spec: When pyproject.toml does not provide a `resources` dir, the rendered specd-orientation.md contains: '<the default `resources` dir + spec-writing-policy.md> (how to write spec items)' [resources.md]
        Spec: When pyproject.toml does not provide a `resources` dir, the rendered specd-orientation.md contains: '<the default `resources` dir + test-writing-policy.md> (reference format and coverage rules)' [resources.md]
        Spec: When pyproject.toml does not provide a `resources` dir, the rendered specd-orientation.md contains: '<the default `resources` dir + spec-implementation-policy.md> (workflow for implementing specs; unenforced)' [resources.md]
        """
        content = _create_and_read_specd_orientation(tmp_path)
        assert (
        ...
        assert (
            "resources/test-writing-policy.md"
            " (reference format and coverage rules)"
            in content
        )
        assert (
            "resources/spec-implementation-policy.md"
            " (workflow for implementing specs; unenforced)"
            in content
        )
