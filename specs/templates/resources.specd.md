...
- When the user calls `resources --copy`, three documents are created: spec-writing-policy.md, test-writing-policy.md, spec-implementation-policy.md
...
- When `resources --copy` runs without error, there is a message: `The following were created:\\n<list of created files>`
- When `resources --copy` runs without error, the message lists created files as `- <target><filename>`

# Target

- If `resources --copy` is called without the `--target` option, the resources are created into the cwd
- If `resources --copy` is called with the `--target` option, the resources are created at the supplied location
...
- If `resources --copy` is called with `--only <resource key>`, only that resource is created
...
- Help text includes: `usage: specd resources [-h] [--list] [--copy] [--only KEY] [--target DIR] [--force]\\n\\n`
...
- Help text includes: `  --copy` + `Copy resource files into a directory`
- Help text includes: `  --only KEY` + `Copy only the named resource (use with --copy)`
