...
- A spec item is any line in a .md file directly or nested in the specd `specs` directory that starts with "- " 
- A line item in the form of a spec item but inside a comment delineated by `<!-- -->` is not a spec item

# Python test/spec discovery
- a python test file is a file directly or nested in the specd `tests` directory named test_*.py
...
- a JS/TS test file is a file directly or nested in the specd `tests` directory named *.test.{js,jsx,ts,tsx}
...
- Each function or method that contains a test-to-spec reference that do not correctly cite a spec is shown on its own lines as: `{n}. {path from tests/ with `::` for internal resource delimiters}\\n   - {the verbatim phantom citation} [{spec file name}]\\n   - {reason}`
...
- The numbers of each line of the summary report represented by `{n}` line up
