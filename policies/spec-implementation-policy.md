...
Note: this workflow is a deliberate departure from classic TDD, suited to multi-agent development where spec-writing, test-writing, and implementation are handled separately and in sequence.
...
During spec implementations, agents are free to consider alternative implementations of existing code, where a refactor would benefit the codebase. But those alternatives should be raised with the user before acting on them.
...
- any test that is inadequate, testing the wrong thing, or testing with the wrong approach
